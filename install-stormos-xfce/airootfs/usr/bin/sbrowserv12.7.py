...
class BrowserCalendar(QWidget):
    ...
    def show_notes_for_date(self, date):
        ...
        if isinstance(note_data, dict):  # Ensure it's a dictionary
            raw_text = note_data.get('raw', '')
        else:
            raw_text = ''  # Default value if the data is invalid
        ...
        if raw_text:
            ...
            <head>
                <style>
                    ...
                    body {{ font-family: sans-serif; font-size: 14pt; color: #f0f0f0; }}
                </style>
            </head>
            ...
            self.notes_text.setHtml('<html><body style="color: white; font-style: ;"></body></html>')
...
class BrowserMainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Setup calendar with clickable URLs in notes
        self.setup_calendar()
    ...
    def toggle_calendar(self):
        """Toggle calendar visibility and display notes for the selected date."""
        if self.calendar_dock.isVisible():
            self.calendar_dock.hide()
        else:
            self.calendar_dock.show()
            # Ensure notes are displayed for the currently selected date
            self.show_notes_for_date(self.calendar_widget.calendar.selectedDate())
...
class StormBrowserDark(BrowserMainWindow):
    ...
    def add_new_tab(self, url=None, title="New Tab", background=False):
        """
        Adds a new tab to the browser with the specified URL and title.

        :param url: The URL to load (str or QUrl). If None, uses the home page.
        :param title: The title of the new tab.
        :param background: If True, the tab is opened in the background without focus.
        :return: Reference to the created QWebEngineView
        """
        # Ensure URL is always a QUrl object
        if url is None:
        ...
        if not hasattr(profile, '_download_handler_connected'):
            ...
            profile._download_handler_connected = True  # Mark as connected
        ...
        browser.page().linkHovered.connect(
            lambda url: self.status_bar.showMessage(url)
        )
        ...
        # Only switch to the new tab if not in background mode
        if not background:
    ...
    def show_settings(self):
        ...
        categories = {
            ...
            "Tools": [
                ...
                ("Calendar", "calendar", "Ctrl+Shift+C"),  # Add this line
            ],
            "Screenshots": [
