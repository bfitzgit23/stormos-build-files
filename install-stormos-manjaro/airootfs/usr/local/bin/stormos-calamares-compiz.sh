#!/usr/bin/env bash
#
# stormos-calamares-compiz.sh — Calamares post-install step (Manjaro XFCE).
# Runs INSIDE the installed (target) system via Calamares' shellprocess,
# with the target mounted at / (dontChroot: false).
#
# Builds compiz-easy-patch from the bundled PKGBUILD (Manjaro does not ship
# compiz in its repos) and deploys the StormOS compiz autostart entry so the
# session can restore Compiz when the user toggles it on.
#
set -uo pipefail
...
cp -r /usr/share/stormos/pkg/compiz-easy-patch "$BUILD_DIR/" 2>/dev/null \
    || cp -r /run/miso/bootmnt/usr/share/stormos/pkg/compiz-easy-patch "$BUILD_DIR/" 2>/dev/null
...
if [ ! -d "$BUILD_DIR/compiz-easy-patch" ]; then
    ...
    [ -d "$BUILD_DIR/sbf/install-stormos-manjaro/airootfs/usr/share/stormos/pkg/compiz-easy-patch" ] && \
        cp -r "$BUILD_DIR/sbf/install-stormos-manjaro/airootfs/usr/share/stormos/pkg/compiz-easy-patch" "$BUILD_DIR/"
fi
...
# --- 3. Deploy autostart entry for every real user home ----------------------
for skel_home in /root /home/*; do
    ...
    chown -R "$(stat -c %U "$skel_home")":"$(stat -c %G "$skel_home")" "$skel_home/.config/autostart" 2>/dev/null || true
done
